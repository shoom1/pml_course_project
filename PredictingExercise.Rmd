...
# will use parallel computation on 6 cores. Comment the next two lines to tun calculations on a single core
...
The number of complete cases in the data is `r sum(complete.cases(df.obs))` out of total `r dim(df.obs)[1]` observations. It means that there is no missing values in the data that is used to build the prediciton model.
...
```
The accuracy of the model based on resampling and bootstrapping the training is `r mean(modelFit$resample$Accuracy)`. OOB estimate of error rate is 0.65%
...
confMatrix$table
```
The accuracy of the prediction on the test set is:  `r confMatrix$overall[1]` with the 95% confidence level [`r confMatrix$overall[3]`, `r confMatrix$overall[4]`]. It is very close to the accuracy based on resampling the training data. Sensitivity and specificity for each class is also quite high - 98-99%:
```{r, echo=FALSE}
confMatrix$overall[c(1,3,4)]
```
```{r, echo=FALSE}
confMatrix$byClass[,c(1,2)]
```
...
modelFits <- lapply(x, function(nt) train(classe ~ ., method="rf", data=training, ntree=nt))
...
g <- ggplot(df.acc, aes(ntree, acc)) + geom_point() + geom_line() + 
  scale_y_continuous(labels = percent) + xlab("Number of Trees") + ylab("Accuracy") + 
  ggtitle("Accuracy of Random Forest vs. Number of Trees") + theme_bw()
...
acc <- sapply(1:100, function(x) {
    inTest1 <- createDataPartition(y = df.obs$classe, p = 0.1, list = FALSE)
    test1 <- df.obs[inTest1,]
    pred1 <- predict(modelFit, test1)
    t = table(pred1, test1$classe)
    sum(diag(t))/sum(t)
  })
...
testing1.obs <- testing1[,columns]
pred1 <- predict(modelFit, testing1.obs)
